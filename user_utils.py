...
class UserInfo:
    ...
    def get_watchlist(self, genre, platform):
        self.watchlist_url = f"https://letterboxd.com/{self.username}/watchlist/"
        if genre != None:
            self.watchlist_url = self.watchlist_url + f"genre/{genre.lower()}/"
        if platform != None:
            self.watchlist_url = self.watchlist_url + f"on/{platform}"

        print(self.watchlist_url)
        page = 2
